...
def main():
    ...
    parser.add_argument(
        "--data_path",
        default="Market_data/Market-1501-v15.09.15/",
        help="path to folder containing train and test datasets (folders named bounding_box_{train/test}/",
    )
    parser.add_argument(
        "--margin", default=0.2, type=float, help="Triplet loss margin."
    )
    parser.add_argument(
    ...
    train_loader = reid_data.get_loader(
        opt.data_path + "bounding_box_train", batch_size=opt.batch_size
    )
    val_loader = reid_data.get_loader(
        opt.data_path + "bounding_box_test", batch_size=opt.batch_size
    )
...
def recall(images, ids):
    ...
    for index in range(images.shape[0]):
        ...
        d = scores[
            index
        ]  # shape (1, batch_size) = distance of positive instances with the rest
        inds = np.zeros(d.shape[0])
        inds = np.argsort(d)[::-1]
        for i, ind in enumerate(inds):
            if (ids[ind] == id) and (
                ind != index
            ):  # on ne regarde pas la distance avec l'élément lui-même
                ranks[index] = i
                break
