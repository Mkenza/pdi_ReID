...
class Extractor(object):
    def __init__(self, model_path, opt, use_cuda=True):
        self.net = ReIdModel(opt)
        self.device = "cuda" if torch.cuda.is_available() and use_cuda else "cpu"

        self.net.load_state_dict(
            torch.load(opt.reidmodel_path, map_location=lambda storage, loc: storage)
        )
        logger = logging.getLogger("root.tracker")
        logger.info("Loading weights from {}... Done!".format(opt.reidmodel_path))
